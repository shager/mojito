...
def main():
    inputfiles = ["jit.time_eval", "simple_bv.time_eval"]
    ...
    for filename in inputfiles:
        ...
        fdes.close()
        lines = content.split("\n")
        time_dict = {1000:[], 3000:[], 5000:[], 7000:[], 9000:[], 11000:[], 13000:[], 
                        15000:[], 17000:[], 19000:[], 21000:[], 23000:[], 25000:[], 27000:[], 
                        29000:[], 31000:[], 33000:[], 35000:[], 37000:[], 39000:[], 41000:[]}
        for key in range(1000, 41001, 2000):
            for line in lines:
                if line == "":
                    continue
                if line.split(",")[0] == str(key):
                    time_dict[key].append(line.split(",")[2])
    ...
    for i in range(1000, 41001, 2000):
        output_string += str(i) + " " #indicate our sample size
        output_string += str(td_list[0][i][0]) + " " #get algorithm 1
        output_string += str(td_list[1][i][0]) + " " #get algorithm 2
        output_string += str(td_list[0][i][1]) + " " #get algorithm 1 error
        output_string += str(td_list[1][i][1]) + " " #get algorithm 2 error
        output_string += "\n"
    print output_string
    fdes = open("time_eval.dat", 'w')
