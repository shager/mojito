...
One popular instance of an SDN protocol is called OpenFlow~\cite{openflow_spec10}. %TODO: hässlicher satz
...
hash maps, decision trees or divide and conquer strategies~\cite{hicuts, efficuts}. %TODO: mehr quotes
...
of bitvectors with fixed length and performs lookups in $\mathcal O(log\ n)$. %TODO: ABV lookup perf. rausfinden
...
Additionally, the rule generator can perform a heuristic analysis of the 
...
gain an even bigger advantage compared to the existing implementations. %TODO: quotes
...
%TODO: bisschen mehr (was wurde getan)
...
With that knowledge, the compiler can produce a function $\textsc{F}'(x)$, as listed in Algorithm~\ref{alg:futamura-f-optimized}.
...
\section{Conclusion}
The implementation of sophisticated algorithms in the OpenFlow reference 
implementation switch indicates a gain in packet classification performance.
...
However, no real advantage is gained if there are no real flows recognizable in the traffic because it is very sparse.
...
small flows, that are very small.
