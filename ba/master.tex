...
Other approaches to this problem include the usage of tries, hashmaps or even geometric algorithms\cite{hicuts} in order to perform faster than searching in a linear list.

The bitvector approach\cite{bv} has been evaluated in FPGAs\cite{bitvector_fpga} \cite{qu2013fast}.
Further improvement on this algorithm includes the elimination of the linear processing of the resulting bitvectors itself:
When all dimensions are matched and the resulting bitvectors are retrieved, these vectors have to be ANDed bit per bit until a 1 is found.
This is obviously an $\mathcal O(n)$ worst case runtime.
