...
implementation of the Software Defined Network OpenFlow. %TODO: OpenFLow is not "a" SDN!
...
Usually, this property of CIDR leads to an increasing size of rule sets in the respective Internet traffic handlers. %TODO: Grafik zum Beispiel?
...
\begin{figure}
\centering
\lstinputlisting[language={[x86masm]Assembler},breaklines=true,basicstyle=\footnotesize,multicols=2,numbers=left]{jit_listing.asm}
\caption{Generated assembly code by the JIT compiler.}
\label{lst:jit}
\end{figure}
...
\Function{JIT\_creation}{delimiters[], low\_index, high\_index}
    ...
    \If{hi\_index $==$ low\_index}
        \State \Call{Generate\_Opcode}{Compare: \textsf{\%rax} $<$ delimiters[low\_index]}
        \State \Call{Generate\_Opcode}{Jump: CASE2}
        \State \Call{Generate\_Opcode}{Return: low\_index}
        \State \Call{Generate\_Opcode}{Return: low\_index - 1} \Comment{CASE2}
    ...
    \State \Call{Generate\_Opcode}{Compare: \textsf{\%rax} $<$ delimiters[mid\_index]}
    \State \Call{Generate\_Opcode}{Jump: LEFT}
    \State \Call{Generate\_Opcode}{Compare: delimiters[mid\_index + 1] $<$ \textsf{\%rax}}
    \State \Call{Generate\_Opcode}{Jump: RIGHT}
    \State \Call{Generate\_Opcode}{Return: mid\_index}
    \State \Call{Generate\_Opcode}{LEFT: left\_tree}
    \State \Call{Generate\_Opcode}{RIGHT: right\_tree}
    \State \Return{Number of written bytes in \textsc{Insert}}
\EndFunction
\end{algorithmic}
...
%Englischtipps: furthermore, whereas
