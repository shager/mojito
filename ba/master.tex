...
\Function{lookup\_matching\_rule}{Packet p}
    ...
    \ForAll{Header fields h $\in$ p}
        %\Comment{Join all bitvectors to get the result}
...
\Function{JIT\_creation}{delimiters[], *executable\_memory, low\_index, high\_index}
    ...
    \If{$hi\_index == low\_index$}
        \State \Call{Insert}{cmpq delimiter[low\_index]}
        \State \Call{Insert}{jb 0x7} \Comment offset = len(mov) + len(pop\_ret)
        \State \Call{Insert}{mov low\_index}
        \State \Call{Insert}{pop\_ret}
        \If{$low\_index == 0$}
            \State \Call{Insert}{mov 0}
            \State \Call{Insert}{pop\_ret}
        \Else
            \State \Call{Insert}{mov low\_index}
            \State \Call{Insert}{pop\_ret}
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Matching performance relative against the List algorithm.}
\end{figure}
...
This test was executed on the two modules separately and in only one matching dimension.
